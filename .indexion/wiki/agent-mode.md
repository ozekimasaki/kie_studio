...
デスクトップ（Electrobun）では `agent/dist/app.mjs` を `Resources/agent-server` として同梱し `loadFlueNodeApplication` で埋め込む。同じ Bun.serve が `/agents/*` を転送する。Electrobun は asar 後に `Resources/app` を削除するため、`postBuild` で `Resources/agent-server` へ退避する（`asarUnpack` だけではインストーラーに残らない）。見つからない場合は `app.asar` から userData/`agent-server` へ展開する。

Windows の bun エントリは `%TEMP%/electrobun-*.js` の Worker として動く。cwd / argv0 / `import.meta.url` が Temp だと `../Resources/agent-server` も Electrobun 自身の `../Resources/version.json` も外れる。起動時に `%LocalAppData%\ai.kie.studio\<ch>\app\bin` へ `chdir` し、同じレイアウトを絶対パスでも探す。`FLUE_DB_PATH` は userData の `flue.db`（`studio.db` と同階層。アンインストールで親 dir を消さない）。
...
`GET /agents/health` で到達性を見る。不通なら AgentView が送信前に警告する。502 本文は Flue の error envelope（`type: agent_unavailable`）にし、SDK が `request failed` に潰さない。sidecar プロキシは起動レース向けに短く再試行する。インストーラー版で embed が無い／読めないときは sidecar（:8789）へは飛ばず、再起動を案内する。Windows では Worker の cwd が Temp でも `%LocalAppData%\ai.kie.studio` 配下の `Resources/agent-server` を探す。desktop:dev（リポジトリ checkout）では従来どおり sidecar `:8789` へフォールバックする。
