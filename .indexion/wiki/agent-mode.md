...
`GET /agents/health` で到達性を見る。不通なら AgentView が送信前に警告する。502 本文は Flue の error envelope（`type: agent_unavailable`）にし、SDK が `request failed` に潰さない。sidecar プロキシは起動レース向けに短く再試行する。インストーラー版で embed が無い／読めないときは sidecar（:8789）へは飛ばず、再起動を案内する。Windows では Worker の cwd が Temp でも `%LocalAppData%\\ai.kie.studio` 配下の `Resources/agent-server` を探す。
